...
namespace SS15.Client.Rendering
{
    public class WorldRenderer
    {
        ...
        private const float MaxVisiblePlasma = 5f;   // уменьшено для лучшей видимости
        private const float FireTemperatureThreshold = 150f; // температура горения в Кельвинах
        ...
        public void Draw()
        {
            ...
            // Плазма и огонь
            DrawGasAndFire(startTileX, startTileY, endTileX, endTileY);
        ...
        private void DrawGasAndFire(int startTileX, int startTileY, int endTileX, int endTileY)
        {
            for (int y = startTileY; y <= endTileY; y++)
            {
                for (int x = startTileX; x <= endTileX; x++)
                {
                    var atmos = _atmosphereSystem.GetAtmosphereAt(x, y);
                    if (atmos == null) continue;

                    // Плазма (фиолетовый полупрозрачный)
                    if (atmos.MolesPlasma > 0.01f)
                    {
                        float alpha = Math.Clamp(atmos.MolesPlasma / MaxVisiblePlasma, 0f, 0.9f);
                        if (alpha >= 0.05f)
                        {
                            Color plasmaColor = new Color(0.8f, 0.2f, 1f, alpha);
                            _spriteBatch.Draw(_whiteTex,
                                new Vector2(x * TileSize, y * TileSize),
                                null, Vector2.One,
                                0, Vector2.Zero, plasmaColor);
                        }
                    }

                    // Огонь (если есть плазма + кислород и высокая температура)
                    if (atmos.MolesPlasma > 0 && atmos.MolesOxygen > 0 && atmos.Temperature >= FireTemperatureThreshold)
                    {
                        float fireAlpha = Math.Clamp((atmos.Temperature - FireTemperatureThreshold) / 300f, 0.3f, 0.9f);
                        Color fireColor = new Color(1f, 0.5f, 0f, fireAlpha);
                        _spriteBatch.Draw(_whiteTex,
                            new Vector2(x * TileSize, y * TileSize),
                            null, Vector2.One,
                            0, Vector2.Zero, fireColor);
                    }
                }
            }
        }
