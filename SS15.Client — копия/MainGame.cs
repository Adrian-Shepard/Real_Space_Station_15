...
namespace SS15.Client
{
    public class MainGame : GraphicsDevice
    {
        ...
        private void SubscribeInputEvents()
        {
            ...
            _inputHandler.ModifyPlasma += OnModifyPlasma;
        }
        ...
        private void OnModifyPlasma(int x, int y, float delta) => _gameState.Atmosphere.ModifyPlasma(x, y, delta);
        ...
        {
            var (playerMs, atmosMs, powerMs, healthMs) = _gameState.Update((float)deltaTime);
            _playerUpdateMs = playerMs;
            _atmosUpdateMs = atmosMs;
            _powerUpdateMs = powerMs;
            _healthUpdateMs = healthMs;
