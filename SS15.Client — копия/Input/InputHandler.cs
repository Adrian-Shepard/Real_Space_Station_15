...
namespace SS15.Client.Input
{
    public class InputHandler
    {
        ...
        public event Action<int, int, float> ModifyPlasma;
        ...
        public void Update(float deltaTime)
        {
            ...
            // Преобразование: фактические экранные координаты -> виртуальные -> мировые
            Vector2 virtualScreen = mouseScreen / scale; // т.к. scale = actual / virtual
            ...
            if (_keyboard.IsKeyPressed(Key.K))
                ModifyTemperature?.Invoke(mouseTileX, mouseTileY, -10f);
            if (_keyboard.IsKeyPressed(Key.Y))
                ModifyPlasma?.Invoke(mouseTileX, mouseTileY, 1f);
